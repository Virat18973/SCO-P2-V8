...
def solve_blend_with_compensation(df,production_tonnes=1000,targets=TARGETS,baseline_blend=None,scenario_tag="base",**kwargs):
    ...
    # Use CBC explicitly and never let a solver failure disappear inside Streamlit.
    try:
        solver=pulp.PULP_CBC_CMD(msg=0, timeLimit=120)
        if hasattr(solver, "available") and not solver.available():
            return "SolverUnavailable",None,None,None,["CBC solver is not available in this deployment. Ensure pulp is installed and its CBC solver is available."],True
        status=prob.solve(solver)
    except Exception as exc:
        return "SolverError",None,None,None,[f"{type(exc).__name__}: {exc}"],True
    st=pulp.LpStatus.get(status,str(status))
    if st!="Optimal":
