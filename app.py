...
def run():
    """Run the optimizer against the currently active/uploaded master.
    Never fail silently: solver errors/status are stored and displayed.
    """
    df=active_df()
    try:
        with st.spinner("Optimizing burden... please wait"):
            res=opt.solve_blend_with_compensation(df,1000,TARGETS,scenario_tag=f"run_{st.session_state.runs+1}")
        status,blend,cost,achieved,diagnostics,fallback=res
        st.session_state.result={"status":status,"blend":blend,"cost":cost,"achieved":achieved,
                                 "diagnostics":diagnostics,"fallback":fallback,"df":df}
        if blend:
            st.session_state.manual_base=dict(blend)
            st.session_state.manual_values=dict(blend)
        else:
            st.session_state.manual_base=None
            st.session_state.manual_values=None
        ...
        st.session_state.result={"status":"ERROR","blend":None,"cost":None,"achieved":None,
                                 "diagnostics":[f"{type(e).__name__}: {e}"],"fallback":True,"df":df}
        return False
...
def dashboard():
    ...
    with u1:
        ...
        if f:
            try:
                sig=(f.name, getattr(f,"size",None))
                # A newly uploaded workbook becomes the active master automatically.
                # Existing edits/toggles are preserved until a genuinely new file is uploaded.
                if st.session_state.upload_signature != sig:
                    ...
                    st.rerun()
                st.success(f"{len(st.session_state.df)} materials loaded from {f.name} — active master")
            except Exception as e:
                st.error(f"Excel validation failed: {type(e).__name__}: {e}")
    with u2:
        st.markdown(kpi("ACTIVE MASTER",st.session_state.source,"single source of truth","good"),unsafe_allow_html=True)
    with u3:
        if st.button("🚀 RUN OPTIMIZER",type="primary",use_container_width=True,key="run_optimizer"):
            ok=run()
            if ok:
                st.success(f"Optimization completed — {st.session_state.result['status']}")
            else:
                st.error("Optimizer failed. See the solver status below.")
            st.rerun()
    if st.session_state.changed: st.warning("Inputs changed. Run optimizer to apply the current availability/prices/limits.")

    r=st.session_state.result
    if r and not r.get("blend"):
        status=r.get("status","UNKNOWN")
        diag=r.get("diagnostics") or []
        st.error(f"Optimizer status: {status}")
        if diag:
            st.code("\n".join(map(str,diag)), language="text")
        st.info("Check that at least one feasible combination remains after availability, technical limits, chemistry constraints, and the 8% IOL / 17% BF Returns mandates are applied.")
...
def settings():
    ...
    if f:
        try:
            sig=(f.name, getattr(f,"size",None), "settings")
            if st.session_state.upload_signature != sig:
                df=opt.load_master_excel(f)
                ...
                st.session_state.upload_signature=sig
                st.rerun()
            st.success(f"{len(st.session_state.df)} materials loaded and active from {f.name}.")
        except Exception as e:
            st.error(f"Excel validation failed: {type(e).__name__}: {e}")
    st.info("Upload becomes the active single source of truth. Review availability/inputs, then return to Dashboard and click Run Optimizer.")
