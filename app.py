...
def dashboard():
    ...
    st.markdown('<div class="panel"><div class="panel-title">RAW MATERIAL INPUTS vs OPTIMIZED BURDEN</div><div class="small">One merged master table: chemistry + moisture + price + stock + Tech Min/Max + availability. Alternative ores are enabled only by the user. The output table uses the same material order.</div></div>',unsafe_allow_html=True)

    material_count=len(st.session_state.master_df.index)
    shared_h=max(260, 34*material_count+45)
    left,right=st.columns([1.55,1],gap="small")

    with left:
        inp=st.session_state.master_df.reset_index()[["Material","Material_Role","Group"]+CHEM_COLS+["Price_Rs_t","Available_Tonnes","Tech_Min","Tech_Max"]].copy()
        inp.rename(columns={"Material_Role":"Role","SiO2":"SiO₂","Al2O3":"Al₂O₃","Moisture_Pct":"Moisture %","Price_Rs_t":"Price ₹/t","Available_Tonnes":"RM Stock t","Tech_Min":"Tech Min","Tech_Max":"Tech Max"},inplace=True)
        inp["Availability"]=[st.session_state.include_alt.get(m,False) if st.session_state.master_df.loc[m,"Material_Role"]=="Alternative_Iron_Ore" else st.session_state.available.get(m,False) for m in inp.Material]
        ed=st.data_editor(inp,key="merged_master_editor",hide_index=True,use_container_width=True,height=shared_h,disabled=["Material","Role","Group"],column_config={
            "Material":st.column_config.TextColumn("Raw Material",width="medium"),"Role":st.column_config.TextColumn("Role",width="small"),"Group":st.column_config.TextColumn("Group",width="small"),
            "Fe":st.column_config.NumberColumn("Fe %",format="%.2f"),"SiO₂":st.column_config.NumberColumn("SiO₂ %",format="%.2f"),"Al₂O₃":st.column_config.NumberColumn("Al₂O₃ %",format="%.2f"),"CaO":st.column_config.NumberColumn("CaO %",format="%.2f"),"MgO":st.column_config.NumberColumn("MgO %",format="%.2f"),"LOI":st.column_config.NumberColumn("LOI %",format="%.2f"),"Moisture %":st.column_config.NumberColumn("Moisture %",format="%.2f"),
            "Price ₹/t":st.column_config.NumberColumn("Price ₹/t",min_value=0,step=1,format="%.0f"),"RM Stock t":st.column_config.NumberColumn("RM Stock t",min_value=0,step=100,format="%.0f"),"Tech Min":st.column_config.NumberColumn("Tech Min",format="%.0f"),"Tech Max":st.column_config.NumberColumn("Tech Max",min_value=0,step=1,format="%.0f"),"Availability":st.column_config.CheckboxColumn("Use / Available")})
        if not ed.equals(inp):
            for _,row in ed.iterrows():
                m=row["Material"]; role=st.session_state.master_df.loc[m,"Material_Role"]
                for src,dst in [("Fe","Fe"),("SiO₂","SiO2"),("Al₂O₃","Al2O3"),("CaO","CaO"),("MgO","MgO"),("LOI","LOI"),("Moisture %","Moisture_Pct"),("Tech Min","Tech_Min"),("Tech Max","Tech_Max"),("Price ₹/t","Price_Rs_t"),("RM Stock t","Available_Tonnes")]:
                    st.session_state.master_df.loc[m,dst]=float(row[src])
                (st.session_state.include_alt if role=="Alternative_Iron_Ore" else st.session_state.available)[m]=bool(row["Availability"])
            st.session_state.changed=True

    with right:
        rh1,rh2=st.columns([1.3,1])
        with rh1: st.markdown('<div class="panel-title" style="margin-top:.4rem">OPTIMIZED BURDEN & COST</div>',unsafe_allow_html=True)
        with rh2: basis=st.radio("Basis",["Dry","Wet"],key="dash_basis",horizontal=True,label_visibility="collapsed")
        if r and r["blend"]:
            if basis=="Wet": out,tot_burden,tot_cost=wet_result_table(r["blend"],r["df"],include_total=True)
            else: out=aligned_result_table(r["blend"],r["df"],include_total=True); tot_burden=float(out.iloc[-1]["kg/t"]); tot_cost=float(out.iloc[-1]["Cost ₹/t"])
            # static table: no internal vertical scrollbar
            st.table(out.round(3))
            t1,t2=st.columns(2)
            t1.metric("TOTAL BURDEN",f"{tot_burden:,.2f} kg/t")
            t2.metric("TOTAL MATERIAL COST",f"₹{tot_cost:,.2f}/t")
        else: st.info("Run the optimizer to populate the optimized burden and cost table.")

    # Specific consumption directly below the two main tables.
    if r and r["blend"]:
        dry_total=float(sum(float(v) for v in r["blend"].values()))
        wet_total=float(wet_result_table(r["blend"],r["df"],include_total=False)[1])
        st.markdown('<div class="panel"><div class="panel-title">SPECIFIC CONSUMPTION</div></div>',unsafe_allow_html=True)
        sc1,sc2=st.columns(2)
        sc1.markdown(kpi("DRY SPECIFIC CONSUMPTION",f"{dry_total:,.2f} kg/t","Optimizer / dry burden basis","s"),unsafe_allow_html=True)
        sc2.markdown(kpi("WET SPECIFIC CONSUMPTION",f"{wet_total:,.2f} kg/t","As-received / wet basis","g"),unsafe_allow_html=True)
    ...
    st.markdown('<div class="small" style="margin-top:.5rem">Edit chemistry directly in the merged master table when the corresponding chemistry columns are enabled in the uploader workflow. Full dry/wet analysis remains available under Recipe & Composition.</div>',unsafe_allow_html=True)
